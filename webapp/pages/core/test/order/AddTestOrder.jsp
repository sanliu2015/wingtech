...
<header style="text-align:center"    data-options="collapsible:true" >Panel Header<select name="orderKind"  data-options="editable:false" id="orderKind" class="easyui-combobox"  style="width:150px;height:30px">   
		...
		<table cellpadding="0" cellspacing="0" class="baseForm-table" width="100%">
			<tr  >
				...
				<td> <select name="bean.orderKind"  id="bean.orderKind" class="easyui-combobox"  data-options="editable:false" style="width:150px;height:30px">   
                                <ts:forEach name='orderKindList'/>
							</select></td>
           </tr> 
           <tr  >
                <td  nowrap><label   for="bean.custName">客户*</label></td>
				<td><input name="bean.customer.name" id="bean.customer.name" class="easyui-textbox"  style="width:150px;height:30px"  /></input><a href="javascript:void(0)" class="easyui-linkbutton"  plain='true' iconCls="icon-search" onClick="return testOrderScript.chooseCust(this);">选择</a><input name="bean.customer.id" id="bean.customer.id" type="hidden"  /></td>
...
<table id="testOrderDtlListGrid"   class="easyui-datagrid" style="width:auto;height:auto;"
		title="产品信息"  data-options="collapsible:true,singleSelect:true,   rowStyler: function(index,row){ return 'height:40'}">
			<thead>
				<tr>
					<th data-options="field:'operateField',width:50,formatter:testOrderScript.removeRowFormat">操作</th>
       ...
       <tr cloneRowFlag="1"  class="haveborder"   style="display:none">
          ...
          <tr cloneRowFlag="1" class="haveborder"  style="display:none" > 
       	    <td colspan="2"> </td>
           <td> 备注：<input name="testOrderFileList.description"   class="easyui-textbox"  style="width:150px;height:30px"  /></input> <select name="testOrderFileList.orderKind"  id="testOrderFileList.orderKind"   data-options="editable:false"  style="width:150px;height:30px"><option value="1">abee</option><option value="3">444</option></select></td>
   ...
   <div id="testOrderFileListToolbar" >
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true"
		 onclick="$.ts.EasyUI.copyTableRow(this,{callbackEvent:parseCombobox})">添加</a>&nbsp;&nbsp; 
	</div> 
...
<script type="text/javascript">
	...
	function parseCombobox(){
		 var len=$("[id='testOrderFileList.orderKind']").length ;
		 if(len>0){
			 $($("[id='testOrderFileList.orderKind']")[len-2]).combobox();
		 }
	}
</script>
</form>
