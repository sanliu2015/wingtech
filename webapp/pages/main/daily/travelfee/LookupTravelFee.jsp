...
<div style="padding:10px;padding-left:10px; font-size:14px">
  <table   border="0" cellspacing="0" cellpadding="0"  width="100%"  align="center">
    ...
    <table   border="0" cellspacing="0" cellpadding="0"  width="100%"  align="center">
    	<tr><td width="25%"  height="30px">动物营养事业部<input name="bean.requestObjectKind" type="checkbox" value="0" onClick="tavelFeeScript.checkSinglebox(this)"></td>
          <td width="25%">动物保健事业部<input name="bean.requestObjectKind" type="checkbox" value="1" onClick="tavelFeeScript.checkSinglebox(this)"></td>
          <td>管理<input name="bean.requestObjectKind" type="checkbox" value="2" onClick="tavelFeeScript.checkSinglebox(this)"></td></tr>
     </table>
      <table   border="0"  cellspacing="0" cellpadding="0" width="100%"  id="dtlListGrid"   align="center" class="billTable" >
		 ...
		 onclick="tavelFeeScript.insertExplanatioRow(this)" tsOptions="needHide:'1'">添加</a></td>
                    <td > Date/日 期</td> 
            ...
            <a href="javascript:void(0)" class="easyui-linkbutton" id="feeSubmitBtn" iconCls="icon-cancel"  onClick=" $.ts.EasyUI.deleteTableRow(this );"  plain=true  style="list-style:none;float:right;text-align:left" tsOptions="needHide:'1'"></a>
      ...
      <tr> 
             <td  ><label  for="bulletinFileList"> 上传附件  </label>
             <table id="fileListGrid">
              <c:forEach items="${form.fileList}" var="file" varStatus="vs">
                ...
                onclick="$.ts.EasyUI.deleteTableRow(this)" tsOptions="needHide:'1'">删除</a><span id="fieldName" style="border:1px solid #03C; background:#FFC">${file.fileName}</span><input type="file" name="fileList.file"   onChange="tavelFeeScript.fileOnChange(this)"   style="display:none" />
		    ...
		    onclick="$.ts.EasyUI.deleteTableRow(this)" tsOptions="needHide:'1'">删除</a><span id="fieldName" style="border:1px solid #03C; background:#FFC"></span><input type="file" name="fileList.file"   onChange="tavelFeeScript.fileOnChange(this)"   style="display:none" />
	...
	formJson.recordOperateStatus="lookup";
	 ...
	 var dtlList=formJson.dtlList;  
	  ...
	  $(function() {   
	   ...
	   var workFlowFlag="${form.workFlowFlag}";
	   if("1"==workFlowFlag){
	   		modalDialogLoadEvent();
	   }
	});
	function lookupWorkFlow(){ 
		$.ts.Utils.lookupHistoricProcess(formJson.bean.processInstanceId);
	...
	function modalDialogLoadEvent() {  
		// $('#dtlListGrid').datagrid('appendRow',{ });
		 ...
		 for(var i=0;i<dtlList.length;i++){ 
		   ...
		   var tmp={"dtlList":dtlList[i],"recordOperateStatus":"lookup"};
		   row.tsLoadData(tmp);  
