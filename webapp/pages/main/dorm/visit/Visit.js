...
var dataOptLeave = {table: "leaveListGrid"};
function VisitScript( ){  
	 this.appKey=""; 
	 this.opts=null;
 }
VisitScript.prototype={      
	...
	textInputFormatLeave : function(val, row, index) {
		if (val == undefined)
			val = "";
		var fieldName = $(this).attr("field");

		return "<input   name='leaveList." + fieldName + "' id='leaveList."
				+ fieldName + "' value='" + val
				+ "'  style='width:100%'  class='easyui-textbox'/>";
	},
	removeRowFormatLeave : function(val, row, index) {
		if (val == undefined)
			val = "";
		var text = '<a href="javascript:void(0)" deleteRecordOperate="1" class="easyui-linkbutton" plain="true" iconCls="icon-cancel"  onClick="$.ts.EasyUI.deleteTableRow(this, dataOptLeave)"></a> ';
		return text;
	},
	hiddenColumnFormatLeave : function(val, row, index) {
		if (val == undefined)
			val = "";
		var fieldName = $(this).attr("field");
		return "<input type='hidden' name='leaveList." + fieldName
				+ "' id='leaveList." + fieldName + "' value='" + val + "'   />";
	},
	insertDatagridRowLeave : function(rowBean) {
		var rowOpt = {};
		if ($.ts.Utils.isEmpty(rowBean) == false) {
			rowOpt = rowBean;
		}
		var table = $('#leaveListGrid');
		table.datagrid('appendRow', rowOpt['leaveList']);
		var ids = document.getElementsByName("leaveList.id");
		var row = $(ids[ids.length - 1]).parent().parent().parent();
		$.parser.parse(row);
		if ($.ts.Utils.isEmpty(rowBean) == false) {
			row.tsLoadData(rowBean);
	...
	appendRowLeave : function() {
		var rowOpt = {};
		rowOpt.recordOperateStatus = "add";
		$("#leaveListGrid").datagrid("appendRow", rowOpt);
		var ids = document.getElementsByName("leaveList.id");
		var row = $(ids[ids.length - 1]).parent().parent().parent();
		$.parser.parse(row);
	},
    ...
    submitForm:function (obj){//点击按钮提交   
		 ...
		 if ($.trim($("#bean\\.doorIn").combobox("getValue")) == "") {
			 $.messager.alert("警告", "进入厂门不能为空!");
			 return false;
		 }
	 ...
	 submitFormOnLeave:function (obj){//点击按钮提交   
		 ...
		 if ($.trim($("#bean\\.doorOut").combobox("getValue")) == "") {
			 $.messager.alert("警告", "离开厂门不能为空!");
			 return false;
		 }
		 ...
		 $.ts.EasyUI.clearElementsSubscript("leaveList.id", $("#leaveListGrid").parent(), null);
		 var index=$.ts.EasyUI.submitRowsToList("leaveList.id",$("#leaveListGrid").parent(),{effectRowFields:["leaveList.recordOperateStatus"]}); 
		 $.ts.EasyUI.ajaxSubmitForm(url,formId,function(){
			 $.ts.EasyUI.closeDialog(obj);
		 }); 
		return false;
