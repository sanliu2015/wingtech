...
<body id="${appReqeustContext.appKey}Body">  
 <form action="${contextPath}/main/departmentService/json/update.do" id="${appReqeustContext.appKey}Form" name="${appReqeustContext.appKey}Form" method="post"> 
...
<div style="padding:10px;padding-left:10%">
		<table cellpadding="0" cellspacing="0" class="baseForm-table">
            ...
            <tr  >
				...
				<td> <select name="bean.company.id"   id="bean.company.id"    class="easyui-combobox"  style="width:180px;height:30px" data-options="editable:false" >   
                                <ts:forEach name='companyList'/>
							</select></td>
           </tr> 
           <tr  >
                <td  nowrap><label   for="bean.principal">负责人</label></td>
				<td nowrap><input name="bean.principal" id="bean.principal" class="easyui-textbox"   style="width:250px;height:30px"   /></input><a href="javascript:void(0)" class="easyui-linkbutton"  plain='true' iconCls="icon-search" onClick="return departmentScript.chooseEmployee(this);">选择</a>
                <input name="bean.principalId"  id="bean.principalId" type="hidden"  />
                </td>
           </tr>  
           <tr  >
           ...
           <tr  >
			...
			<tr>
                ...
                <input type="radio" name="bean.productDept"  value="0"   />否</td> 
			</tr>
            
            <tr  >
                <td  nowrap><label   for="bean.parentName">上级部门名称</label></td>
				<td><input name="bean.parentId" id="bean.parentId" class="easyui-textbox"  style="width:180px;height:30px"  /></input>  </td>
           </tr> 
            <tr  >
            <td  nowrap><label   for="bean.divisionKind">部门类型</label></td>
				<td> <select name="bean.divisionKind"  id="bean.divisionKind" class="easyui-combobox"  data-options="editable:false"  style="width:180px;height:30px" > 
                              <ts:forEach name='divisionKindList' value='${form.bean.divisionKind}' />
							</select></td>
                    </tr>
           <tr  >
...
<script type="text/javascript">
	...
	$(function() { 
	   ...
	   departmentScript.initPage({contextPath:"${contextPath}",appKey:"${appReqeustContext.appKey}"}); 
	  
	});
	function modalDialogLoadEvent() {
		 
		
		//departmentScript.changeCompany();
