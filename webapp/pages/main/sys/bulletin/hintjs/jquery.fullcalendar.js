...
(function ($) {
	...
	function show(target) {
		...
		for (var i = 0; i < weeks.length; i++) {
			...
			for (var j = 0; j < week.length; j++) {
				var day = week[j];
				var dayHtml = '<font size="3" face="Arial Black">' + day[2];
				...
				if (opts.month == day[1]) {
					...
					if (info.color) {
						...
						dayHtml = '<font size="3" face="Arial Black" ' + color + '>' + day[2];
						
					}
					dayHtml += '</font>';
					dayHtml += "<font id='eventContainer'  color='#000099' style='font-size:10pt'  ></font><br/>";
					if (opts.lunarDay) {
						//dayHtml += "<br/><font id='eventContainer'  color='#000099' style='font-size:10pt'  ></font>";
						if (opts.solarTerms && info.solarTerms) {
							dayHtml += '<font color="#BE77FF">' + info.solarTerms + '</font>';
						} else {
							dayHtml += '<font size="2" style="font-size:8pt">' + cDay(info.lDay, info.lMonth) + '</font>';
						}
						
					}
					dayHtml += "<br/><font  color='#BE77FF' style='font-size:8pt' > ";
					if(info.lunarFestival || info.solarFestival){
						if(info.lunarFestival)
						dayHtml += info.lunarFestival;
						if(info.solarFestival)
						dayHtml += info.solarFestival;
					}
					dayHtml +="</font>";
				} 
				var day = $('<td class="calendar-day fullcalendar-day calendar-other-month" style="width:14%"  ></td>').data('info', info).attr('abbr', day[0] + ',' + day[1] + ',' + day[2]).html(dayHtml).appendTo(tr);
				...
				day.hover(function (e) {
					...
					if (inf) {
						...
						var hintStr=$(this).attr("hintStr");
						if(hintStr!=null){
							hintStr='<font color="#ffffff" style="font-size:12pt;">' +hintStr+ '</font>'; 
							//detail.html(ct);
							hintStr=hintStr ;
							detail.html( hintStr); 
	...
	$('head').append('<style>.fullcalendar-day {border-left: 1px dotted green;border-top: 1px dotted green;} '
		 ...
		 + '.fullcalendar-detail{border-radius:5px;position: absolute; z-index: 10; background-color: rgb(0, 0, 102); display: none; opacity: 0.8; padding: 5px; text-align: right;filter: Alpha(opacity=80);width:130px;}'
