...
$.extend($.fn.tabs.methods, {
    ...
    loadTabIframe:function(jq,params){
        return jq.each(function(){
            ...
            var $maskMessage = $('<div class="mask-message" style="z-index:3;width:auto;height:16px;line-height:16px;position:absolute;top:50%;left:50%;margin-top:-20px;margin-left:-92px;border:2px solid #d4d4d4;padding: 12px 5px 10px 30px;background: #ffffff url(\''+params.iframe.image+'\') no-repeat scroll 5px center;">' + (params.iframe.message || '加载中......') + '</div>').appendTo($tabBody);
            ...
            if (iframe.attachEvent){
                iframe.attachEvent("onload", function(){
					if(typeof( iframe.contentWindow.showAlertMsgEvent)!="undefined" && $.isFunction( iframe.contentWindow.showAlertMsgEvent) ){ 
					 
					   iframe.contentWindow.showAlertMsgEvent() ;
					   $('#tabs').tabs('close',params.which );
					   return;
					}
