...
(function($) {
	...
	var handler = {
		...
		onClickNode: function (event) {
			var setting = settings[event.data.treeObjId];
			var treeNode = event.data.treeNode;
			if (tools.apply(setting.callback.beforeClick, [setting.treeObjId, treeNode], true) == false) { 
				return;
			}
		...
		onMousedownNode: function(eventMouseDown) {
			...
			//右键、禁用拖拽功能 不能拖拽 去掉|| !setting.editable
			if (eventMouseDown.button == 2   || (!setting.dragCopy && !setting.dragMove)) { 
				return;
			}
			//编辑输入框内不能拖拽节点
