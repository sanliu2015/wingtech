...
public class TilesConfigurer implements ServletContextAware, InitializingBean, DisposableBean {
	...
	private class SpringTilesContainerFactory extends BasicTilesContainerFactory {
		...
		public TilesContainer createContainer(ApplicationContext context) {
			TilesContainer container = super.createContainer((TilesApplicationContext) context);
			return (useMutableTilesContainer ? new CachingTilesContainer() : container);
		}
		...
		protected List<ApplicationResource> getSources(ApplicationContext applicationContext) {
			if (definitions != null) {
				...
				return null;//super.getSources(applicationContext);
			}
		}

		protected BaseLocaleUrlDefinitionDAO instantiateLocaleDefinitionDao(ApplicationContext applicationContext,
				LocaleResolver resolver) {
			BaseLocaleUrlDefinitionDAO dao =  instantiateLocaleDefinitionDao(applicationContext, resolver);
		...
		protected DefinitionsReader createDefinitionsReader(ApplicationContext context) {
			DigesterDefinitionsReader reader = (DigesterDefinitionsReader) createDefinitionsReader(context);
			//((Object) reader).setValidating(validateDefinitions);
			return reader;
		}
		...
		protected PreparerFactory createPreparerFactory(ApplicationContext context) {
			if (preparerFactoryClass != null) {
				...
				return createPreparerFactory(context);
			}
		}
