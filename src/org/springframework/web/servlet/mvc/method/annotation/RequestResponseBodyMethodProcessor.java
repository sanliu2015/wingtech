...
import com.ts.core.system.module.ModuleForm;
...
public class RequestResponseBodyMethodProcessor extends AbstractMessageConverterMethodProcessor {
	...
	private Type getFormType(NativeWebRequest request,MethodParameter parameter){
		...
		if(SimpleCachePool.serviceParameterMap.containsKey(paraKey)){
			if(SimpleCachePool.serviceParameterMap.get(paraKey)!=null)
				return SimpleCachePool.serviceParameterMap.get(paraKey).getParameterizedType();
			else
				...
				if(method != null){ 
					...
					for(int i=0;i<params.length;i++){ 
						if(IBaseModel.class.isAssignableFrom(method.getParameters()[i].getType() )==true){
							parameter.setParameterClassName(method.getParameters()[i].getType().getName());
							SimpleCachePool.serviceParameterMap.putIfAbsent(paraKey, method.getParameters()[i] ); 
							return method.getParameters()[i].getParameterizedType();
						}
					}
				}
			} 
	...
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer,
		...
		if(parameter.getParameterType()!=null && IBaseModel.class.isAssignableFrom(parameter.getParameterType())){
			type=getFormType(webRequest,parameter);
		}
