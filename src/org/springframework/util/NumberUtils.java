...
public abstract class NumberUtils {
	...
	public static <T extends Number> T parseNumber(String text, Class<T> targetClass) {
		...
		if (targetClass.equals(Byte.class)) {
			...
			if(isPercent){
				return (T) (isHexNumber(trimmed) ? new Integer(new Double(Integer.decode(trimmed)*0.01).intValue()): new Integer(new Double(Integer.valueOf(trimmed)*0.01).intValue()));
			} else
				return (T) (isHexNumber(trimmed) ? Integer.decode(trimmed): Integer.valueOf(trimmed));
		}
		else if (targetClass.equals(Long.class)) {
			if(isPercent) {
				return (T) (isHexNumber(trimmed) ? new Long(new Double(Long.decode(trimmed)*0.01).longValue()) :new Long(new Double(Long.valueOf(trimmed)*0.01).longValue()));
			} else
				return (T) (isHexNumber(trimmed) ? Long.decode(trimmed) : Long.valueOf(trimmed));
		}
		else if (targetClass.equals(BigInteger.class)) {
			if(isPercent) {
				return (T) (isHexNumber(trimmed) ? new BigInteger(new Double(decodeBigInteger(trimmed).doubleValue()*0.01).toString()) : new BigInteger(new Double(new BigInteger(trimmed).doubleValue()*0.01).toString()));
			} else 
				return (T) (isHexNumber(trimmed) ? decodeBigInteger(trimmed) : new BigInteger(trimmed));
		}
		else if (targetClass.equals(Float.class)) {
			if(isPercent) {
				return (T) new Float(Float.valueOf(trimmed)*0.01);
			} else 
				return (T) Float.valueOf(trimmed);
		}
		else if (targetClass.equals(Double.class)) {
			if(isPercent) {
				return (T) new Double(Double.valueOf(trimmed)*0.01);
			} else 
				return (T) Double.valueOf(trimmed);
		}
		else if (targetClass.equals(BigDecimal.class) || targetClass.equals(Number.class)) {
			if(isPercent) {
				return (T) new BigDecimal(trimmed).subtract(new BigDecimal("0.01"));
			} else 
