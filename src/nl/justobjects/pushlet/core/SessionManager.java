...
public class SessionManager implements ConfigDefs {
  ...
  public Session createSession(Event anEvent) throws PushletException {
    ...
    //return Session.create(createSessionId()); 
	  ...
	  parameters[0][0]="loginstatus";
	  parameters[0][1]=anEvent.getField("loginstatus");
	  Session session=Session.create(anEvent.getField("userId", "visitor"));
  ...
  public Session removeSession(Session aSession) {
    synchronized (mutex) {
      ...
      if (session != null) {
        ...
        user.setId(new Integer(session.getId()));
        String[][] ps=session.getParameters();
        user.setLoginstatus("pc");
        if(ps!=null){
        	for(int i=0;i<ps.length;i++){
        		String[] row=ps[i];
        		if(row[0].equals("loginstatus")){
        			if(row[1]!=null && !row[1].equals(""))
        				user.setLoginstatus(row[1]);
        		}
        	}
        }
        
        SessionListener.removeLogin(user, null);
        RequestContext rc=new RequestContext();
        rc.setUser(user);
        pushOnLinePers(rc,AppContext.getBaseService());
      }
      sessionCacheDirty = true;
      return session;
    }
  }
  private void pushOnLinePers(RequestContext requestContext, IBaseServiceManger service){
		Map eventParam=new HashMap();
		eventParam.put("userIds", "all");
		eventParam.put("msg", SessionListener.loginMap.size()+"");
		service.getPushlet().publishMsg("/ts/login", Globals.PushEvent.multicast, eventParam, requestContext, service);
	}
  /**
   * Starts us.
   */
