...
@Controller
@RequestMapping(value = "/"+Globals.URL_COREPREFIX+"/**")
public class CoreController {
	 
	/**
	 * 若默认为jackson解析json，不标明produces = "application/json;charset=UTF-8"，则返回的是xml格式，
	 * 若为fastjson解析json,不标明produces = "application/json;charset=UTF-8"，则返回的为null，
	 */
    ...
    public  Object returnJsonContent(NativeWebRequest nativeWebRequest, HttpServletRequest request,HttpServletResponse response,RedirectAttributesModelMap attr,@PathVariable(value="methodName") String methodName,IBaseModel form) throws Exception{    
		InvocableMethodHandler handler=new InvocableMethodHandler(Globals.URL_JSONTYPE,Globals.URL_COREPREFIX,methodName,nativeWebRequest,request,response,attr,form);		
    ...
    public Object recJsonAction(NativeWebRequest nativeWebRequest,HttpServletRequest request,HttpServletResponse response,RedirectAttributesModelMap attr,@PathVariable(value="methodName") String methodName, @RequestBody(required=false)IBaseModel  form) throws Exception{
		InvocableMethodHandler handler=new InvocableMethodHandler(Globals.URL_RECJSONTYPE,Globals.URL_COREPREFIX,methodName,nativeWebRequest,request,response,attr,form);		
		Object obj=handler.executeMethod(); 
        return obj;
    }
	@RequestMapping(value = "/"+Globals.URL_RECRTOJSONTYPE+"/{methodName}."+Globals.URL_SUBFIX)//, produces = "application/json;charset=UTF-8"
	@ResponseBody
    public Object recAndReturnJson(NativeWebRequest nativeWebRequest,HttpServletRequest request,HttpServletResponse response,RedirectAttributesModelMap attr,@PathVariable(value="methodName") String methodName, @RequestBody(required=false)IBaseModel  form) throws Exception{
		InvocableMethodHandler handler=new InvocableMethodHandler(Globals.URL_RECRTOJSONTYPE,Globals.URL_COREPREFIX,methodName,nativeWebRequest,request,response,attr,form);		
		Object obj=handler.executeMethod();
        return obj;
    }
    ...
    public  void requestStream(NativeWebRequest nativeWebRequest, HttpServletRequest request,HttpServletResponse response,RedirectAttributesModelMap attr,@PathVariable(value="methodName") String methodName,IBaseModel form) throws Exception{    
		InvocableMethodHandler handler=new InvocableMethodHandler(Globals.URL_STREAMTYPE,Globals.URL_COREPREFIX,methodName,nativeWebRequest,request,response,attr,form);		
    ...
    public Object execute(NativeWebRequest nativeWebRequest,HttpServletRequest request,HttpServletResponse response,RedirectAttributesModelMap attr,@PathVariable(value="methodName") String methodName, IBaseModel  form) throws Exception{
		InvocableMethodHandler handler=new InvocableMethodHandler(Globals.URL_EXECUTETYPE,Globals.URL_COREPREFIX,methodName,nativeWebRequest,request,response,attr,form);		
		Object obj=handler.executeMethod(); 
        return obj;
    }
